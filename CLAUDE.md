...
481 tests, fast by design. New PRs must keep the suite green.

`engine/tests/conftest.py` stubs the batched Reddit prefetch out of every test, so the suite never reaches the network. A test that exercises the prefetch itself opts in with `@pytest.mark.live_prefetch` and mocks the transport (`urllib.request.urlopen`) directly.
...
- **Reddit RSS rate-limit discipline** (`reddit.py`). The keyless RSS surface enforces a per-IP token bucket. As of **2026-08-11** that bucket is **one request per ~60s window**, shared across `www.reddit.com` and `old.reddit.com` (verified: a second GET to either host inside the window is a hard 429). Every 200 therefore reports `x-ratelimit-remaining: 0` / `used: 1`. Three rules follow, and breaking any of them breaks every run:
  - **`remaining=0` on a 200 is the steady state, not a fault.** Pause until reset, then CONTINUE. It must never flip a terminal flag. It did until 2026-08-11, which is why runs read exactly one subreddit and reported `rate_limited` with 9 subs skipped, on every single run. A 429 that a retry recovers from clears the flag too; only a 429 that survives every retry is a real stop.
  - **Sub feeds are batched, not per-sub.** `plan_sub_batches` packs subs into combined `/r/a+b+c/new/.rss` feeds (one request each, entries tagged by `<category term>`), so a 10-sub run costs ~4 requests instead of 10. Packing is by `saturation` cost under `BATCH_COST_CAP` so a busy sub cannot crowd a quiet one out of the shared 100-entry page. `prime_new_cache` runs before the sub loop and `fetch_delta` reads it, so the per-sub call and its delta/cursor contract are unchanged. Search-driven modes hit `search.rss` with a per-sub query and cannot share a feed, so they stay on the per-sub path (and keep the mid-loop `is_rate_limited()` break).
  - **Every run carries a GET budget** (`set_request_budget`, default `DEFAULT_REQUEST_BUDGET`, override `--max-requests`). At ~1 request/minute the budget IS the run's worst-case wall-clock in minutes, and it is what keeps a run bounded instead of open-ended. Sub feeds draw first; the author vet checks `budget_exhausted()` and fails open (uncached, since a skip is not a verdict) rather than adding a minute per OP. A budget skip is counted as neither `ok` nor `failed`, so it can never masquerade as a `blocked` run.

  The run JSON reports three states: `ok`, `rate_limited` (transient, retry shortly), `blocked` (non-429 edge failure), plus `reddit_requests` and `subs_scanned` so a partial run is legible. No OAuth, no API key.
